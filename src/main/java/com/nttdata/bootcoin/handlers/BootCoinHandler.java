package com.nttdata.bootcoin.handlers;
...
public class BootCoinHandler {
  ...
  public Mono<ServerResponse> buyBootCoin(ServerRequest request) {
    ...
    return bootCoinMono.flatMap(b -> bootCoinServices
        .findByPhoneNumber(phoneNumber).flatMap(bootCoin -> yankiDTOService
            .findByPhoneNumber(bootCoin.getPhoneNumber()).flatMap(yanki -> {
              ...
              return yankiDTOService.updateYanki(yanki)
                  ...
                  .flatMap(y -> {
                    return transactionService.save(BootCoinTransactionDTO
                        ...
                        .exchange(b.getAmount() * COMPRA).typeTransaction("BUY")
                        .phoneNumber(y.getPhoneNumber()).build());
                  });
            })))
        .flatMap(
            c -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON)
            ...
            .findByPhoneNumber(bootCoin.getPhoneNumber()).flatMap(yanki -> {
              ...
              return yankiDTOService.updateYanki(yanki)
                  ...
                  .flatMap(y -> {
                    return transactionService.save(BootCoinTransactionDTO
                        .builder().amount(b.getAmount())
                        .exchange(b.getAmount() * VENTA).typeTransaction("SELL")
                        .phoneNumber(y.getPhoneNumber()).build());
                  });
            })))
  ...
  /*
   * public Mono<ServerResponse> buyBootCoin(ServerRequest request){
   * Mono<BootCoin> bootCoinMono = request.bodyToMono(BootCoin.class); String id
   * = request.pathVariable("id"); return
   * bootCoinServices.findById(id).zipWith(bootCoinMono, (db,req) -> {
   * db.setAmount(req.getAmount()); return db; }).flatMap( c -> ServerResponse
  ...
  public Mono<ServerResponse> findByPhoneNumber(ServerRequest request) {
    ...
    return ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(
        bootCoinServices.findByPhoneNumber(bootCoinNumber), BootCoin.class);
  }
  ...
  public Mono<ServerResponse> newAccountBootCoin(ServerRequest request) {
    ...
    return bootCoinMono
        .flatMap(bootCoin -> bootCoinServices.createBootCoin(bootCoin))
        .flatMap(
            c -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON)
                .body(BodyInserters.fromValue(c)))
  ...
  public Mono<ServerResponse> delete(ServerRequest request) {
    ...
    return accountMono
        ...
        .flatMap(c -> bootCoinServices.delete(c)
